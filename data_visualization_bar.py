...
filtered_class_counts.plot(
    kind="barh",
    color=plt.cm.viridis_r(filtered_class_counts / max(filtered_class_counts)),
)
...
for index, value in enumerate(filtered_class_counts):
    plt.text(
        value, index, str(value), va="center", ha="left", fontsize=12, color="black"
    )
