...
def extract_embeddings(image_paths, batch_size, checkpoint_interval, output_dir='embeddings'):
    ...
    for batch_images, batch_paths in tqdm(dataloader, desc="Overall Progress"):
        ...
        with torch.no_grad():
            batch_embeddings = model(batch_images).squeeze().cpu().numpy()  # Ensure embeddings are moved back to CPU
        ...
        if len(embeddings_with_ids) % checkpoint_interval < batch_size:
            ...
            checkpoint_path = os.path.join(output_dir, f'embeddings_checkpoint_{len(embeddings_with_ids) // checkpoint_interval}.pkl')
            with open(checkpoint_path, 'wb') as f:
                pickle.dump(embeddings_with_ids, f)
            checkpoint_end_time = time.time()
            print(f"Checkpoint saved: {checkpoint_path}")
            print(f"Time spent on checkpointing: {checkpoint_end_time - checkpoint_time:.2f} seconds")
